...
But that chain has to be anchored somewhere: Maybe Charlie vouches for Bob to Alice, but
...
<table>
  ...
  <tbody>
    <tr>
      <td></td>
      <td>
      <img src="$$/logo.pgp.png" className="size-[24px] md:size-[36px] inline" alt="PGP" />
      <img src="$$/logo.keybase.png"  className="size-[24px] md:size-[36px] inline"  alt="Keybase" />
      </td>
      <td>
      <img src="$$/logo.signal.png" className="size-[24px] md:size-[36px] inline" alt="Signal" />
      <img src="$$/logo.whatsapp.png"  className="size-[24px] md:size-[36px] inline"  alt="WhatsApp" />
      <img src="$$/logo.telegram.png" className="size-[24px] md:size-[36px] inline" alt="telegram" />
      </td>
    </tr>
    <tr>
...
If I create a group, by definition I get to decide who's in it. At the moment of
...
The Seitan protocol, like any other flavor of PAKE, is complex
and involves many cryptographic subtleties. However, at its core, it has an interaction that takes
place in the context of some existing relationship.
...
Relying on a "pre-existing side channel" is not a cheat or a cop-out, even though it felt like it at
first. And it's not a minor thing or an implementation detail: It's the core of this whole thing.
...
creating something inherently makes it yours and gives you authority over it.
...
This doesn't automatically make you a despot — it's up to you whether and how you want to delegate
that authority — but it gives you that authority.

Authority is rooted in authorship, in the **act of creation**.

These are the answers I've found to our big questions. I think they're good answers. As it turns
...
sleeve that solves the exact thing you're stuck on.
